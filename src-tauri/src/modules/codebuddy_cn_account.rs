...
const ACCOUNT_STORE_PLATFORM: &str = "codebuddy_cn";
...
fn account_index_from_store() -> Result<CodebuddyAccountIndex, String> {
    ensure_account_store_migrated()?;
    let accounts =
        crate::modules::account_store::list_accounts::<CodebuddyAccount>(ACCOUNT_STORE_PLATFORM)?;
...
pub fn load_account(account_id: &str) -> Option<CodebuddyAccount> {
    if let Err(err) = ensure_account_store_migrated() {
        logger::log_warn(&format!(
            "[CodeBuddy CN Account][Store] 账号数据库迁移检查失败，回退文件读取: account_id={}, error={}",
            account_id, err
        ));
    } else if let Ok(Some(account)) = crate::modules::account_store::load_account::<CodebuddyAccount>(
        ACCOUNT_STORE_PLATFORM,
        account_id,
    ) {
        return Some(account);
...
fn load_account_index() -> CodebuddyAccountIndex {
    match account_index_from_store() {
        ...
        Err(error) => logger::log_warn(&format!(
            "[CodeBuddy CN Account][Store] 从 SQLite 读取账号索引失败，回退 JSON: {}",
            error
        )),
    }
...
fn load_account_index_checked() -> Result<CodebuddyAccountIndex, String> {
    match account_index_from_store() {
        ...
        Err(error) => logger::log_warn(&format!(
            "[CodeBuddy CN Account][Store] 从 SQLite 读取账号索引失败，继续检查 JSON: {}",
            error
        )),
    }
