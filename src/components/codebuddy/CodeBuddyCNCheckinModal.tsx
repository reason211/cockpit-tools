...
import { X, Gift, CheckCircle, XCircle, Loader2, RefreshCw, CalendarCheck, Flame, Trophy, Info } from 'lucide-react';
import { CodebuddyAccount, getCodebuddyAccountDisplayEmail } from '../../types/codebuddy';
import * as codebuddyCnService from '../../services/codebuddyCnService';
import type { CheckinStatusResponse, CheckinResponse } from '../../services/codebuddyCnService';
...
export function CodeBuddyCNCheckinModal({ accounts, onClose, onCheckinComplete }: CodeBuddyCNCheckinModalProps) {
  ...
  const fetchAllStatus = useCallback(async () => {
    ...
    await Promise.allSettled(
      accounts.map(async (account) => {
        try {
          const status = await codebuddyCnService.getCheckinStatusCodebuddyCn(account.id);
  ...
  const handleSingleCheckin = useCallback(async (accountId: string) => {
    ...
    try {
      const result = await codebuddyCnService.checkinCodebuddyCn(accountId);
      updateAccountState(accountId, { checkingIn: false, checkinResult: result });

      if (result.success) {
        // 签到成功后刷新该账号状态
        try {
          const status = await codebuddyCnService.getCheckinStatusCodebuddyCn(accountId);
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content checkin-modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h2><CalendarCheck size={20} /> {t('codebuddyCn.checkin.modalTitle', '每日签到')}</h2>
          <button className="modal-close" onClick={onClose} aria-label={t('common.close', '关闭')}><X size={18} /></button>
        </div>
                  ...
                  <div key={account.id} className={`checkin-account-row ${isCheckedIn ? 'checked' : ''}`}>
                    ...
                    <div className="checkin-account-action">
                      {isCheckingIn ? (
                        ...
                        <button
                          className="btn btn-primary btn-sm"
                          onClick={() => handleSingleCheckin(account.id)}
                        >
                          <Gift size={14} />
                    ...
                    {state?.checkinResult && !state.checkinResult.success && state.checkinResult.message && (
                      <div className="checkin-account-info-msg">
                        <Info size={12} /> {state.checkinResult.message}
                      </div>
                    )}
